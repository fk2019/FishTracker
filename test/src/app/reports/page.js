...
import FeedChart from '../../components/FeedingChart';
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    const gradingData = JSON.parse(localStorage.getItem('gradingData') || '[]');

    // 🟦 Feeding Stats
    const totalFeedings = feedingLogs.length;
    const totalFeedQuantity = feedingLogs.reduce((sum, entry) => sum + Number(entry.quantity || 0), 0);
    ...
    // 🟩 Grading Stats
    let lastGradingAvgWeight = 'N/A';
    if (gradingData.length > 0) {
      const lastGrading = gradingData[gradingData.length - 1];
      lastGradingAvgWeight = `${lastGrading.weight}`;
    }
