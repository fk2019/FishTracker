...
export default function GradingTrackerPage() {
  ...
  const handleSubmit = (e) => {
    ...
    const updatedPond = {
      ...freshPond,
      gradings: [...prevGradings, newGrading]
    }
  ...
  const recentGradings = Object.values(groupByPond);
  //chart
  useEffect(() => {
    const savedPonds = JSON.parse(localStorage.getItem('ponds')) || [];
  ...
  return (
    ...
    className="w-full border p-2 rounded text-sm text-blue-500"
      ...
      <label className="text-lg font-semibold text-blue-800 mb-2">Total Weight ({unit})</label>
        ...
        <div>
          <label className="text-lg font-semibold text-blue-800 mb-2">Number of Fish Graded</label>
          <input
            type="number"
            name="count"
    ...
    onChange={handleChange}
     ...
     {pond && <GradingChart gradings={pond.gradings} />}
    </div>
      </div>
  );
}
