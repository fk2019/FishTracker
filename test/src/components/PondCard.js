export function PondCard({ pond }) {
  return (
    <div className="bg-white rounded-lg shadow p-4 hover:shadow-md transition">
      ...
      <p className="text-sm text-gray-500">Size: {pond.size} m²</p>
      <p className="text-sm text-gray-600">Stocked: {pond.stockingDate}</p>
      <p className="text-sm text-gray-600">Fish: {pond.fishCount}</p>
      <p className="text-sm text-gray-600">Species: {pond.species}</p>
      <p className={`text-sm mt-2 font-bold ${pond.status === 'active' ? 'text-green-600' : 'text-red-600'}`}>
